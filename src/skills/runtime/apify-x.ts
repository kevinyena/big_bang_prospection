...
export interface SearchXUsersParams {
  ...
  maxItems: number;
  /**
   * Drop any returned user whose followers count exceeds this. X People search
   * doesn't expose a max_followers operator, so we apply it immediately after
   * the Apify response. Profiles missing followers_count are conservatively
   * KEPT (we don't know, can't reject).
   */
  maxFollowers?: number;
}
...
export async function searchXUsersByQuery(
  ...
  // Upstream follower-count filter. Applied here (right after Apify returns)
  // so the rest of the pipeline never even sees the over-threshold profiles.
  // X People search doesn't expose a max_followers query operator, so this is
  // the earliest possible filter point. Users with NO followers field
  // (undefined) are conservatively kept — we can't reject what we don't know.
  let users = rawItems;
  if (typeof params.maxFollowers === 'number') {
    const maxF = params.maxFollowers;
    users = rawItems.filter((u) => {
      const fc = u.followers ?? u.followersCount;
      if (typeof fc !== 'number') return true; // unknown → keep
      return fc <= maxF;
    });
  }

  return { users, runId, costUsdActual: actualCostUsd, statusMessage: pollStatusMessage };
}
