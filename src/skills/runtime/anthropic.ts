...
export async function callDirectAnthropic({
  ...
  // Support both plain string and rich content blocks (text + images)
  ...
  const response = await getClient().messages.create({
    ...
    ...(builtTools ? { tools: builtTools as any } : {}),
    ...(toolChoice ? { tool_choice: { type: 'tool', name: toolChoice.tool.name } } : {}),
  });
