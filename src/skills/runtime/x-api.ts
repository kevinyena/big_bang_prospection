...
// Migration aftermath: X.com is the canonical domain since 2023, but the
// OAuth page on twitter.com has a broken session-cookie handshake — login
// succeeds, session cookie is set on x.com (not twitter.com), and the
// twitter.com OAuth page can't read it → infinite "Log in" loop. The fix
// is to send users directly to x.com OAuth, which DOES read the x.com
// session correctly. Requires the user to be logged in to x.com in the
// browser before clicking Link — which is the default since x.com is now
// the only entry point for normal X usage.
