...
import { checkQuotaOk, recordSuccess, recordFailure } from '../runtime/x-dm-quota.js';
...
const HARD_CAP = 30;
// 3-6 minutes between successful sends. Cold outreach at burst rates kills
// the X trust score (we observed cascading 403s after ~10 fast DMs even with
// 15-30s gaps). This pace looks like a human manually DMing throughout the
// day and stays under X's anti-spam ML thresholds.
const DEFAULT_DELAY_MIN_MS = 180_000;  // 3 min
const DEFAULT_DELAY_MAX_MS = 360_000;  // 6 min
...
export const SendXDMsInputSchema = z.object({
  ...
  /**
   * Daily DM cap. If the persistent quota file shows we've already sent this
   * many today (UTC day), the loop refuses to start. Default 50.
   */
  dailyLimit: z
    .number()
    .int()
    ...
    .max(500)
    .default(50)
    .describe('Cap journalier de DMs réussis (UTC day). Bloque si déjà atteint.'),
  /**
   * Consecutive failures threshold that triggers a cooldown. Default 5.
   * If we hit 5 back-to-back 403s, we assume the X trust score is throttled
   * and stop the run (refuse to send more for cooldownMinutes).
   */
  consecutiveFailsThreshold: z
    .number()
    .int()
    .min(1)
    .max(20)
    .default(5)
    .describe('Nb d\'échecs consécutifs avant de déclencher un cooldown stop-loss.'),
  cooldownMinutes: z
    .number()
    .int()
    .min(5)
    .max(24 * 60)
    .default(60)
    .describe('Durée du cooldown stop-loss après la cascade d\'échecs (minutes).'),
});
...
export type SendProgressEvent =
  | { kind: 'start'; total: number; targetSuccesses: number; variants: string[]; delayMinMs: number; delayMaxMs: number; quotaSentToday: number; dailyLimit: number }
  ...
  | { kind: 'skipped'; index: number; handle: string; reason: 'target_reached' | 'cooldown_triggered' | 'daily_limit' }
  | { kind: 'quota'; sentToday: number; dailyLimit: number; consecutiveFails: number; cooldownUntil: number | null }
...
export async function runSendDMs(
  ...
  const dailyLimit = input.dailyLimit ?? 50;
  const consecutiveFailsThreshold = input.consecutiveFailsThreshold ?? 5;
  const cooldownMinutes = input.cooldownMinutes ?? 60;

  // ── Quota gate at the very start of a run ──────────────────────
  const quotaCheck = await checkQuotaOk(dailyLimit);
  if (!quotaCheck.ok) {
    // We refuse the entire run. The caller (UI) gets a `done` with a
    // descriptive failReason on every handle.
    const reasonText =
      quotaCheck.reason === 'cooldown'
        ? `Cooldown actif jusqu'à ${new Date(quotaCheck.until ?? 0).toISOString()} (trust score X dans le rouge, on attend pour ne pas aggraver).`
        : `Daily limit atteint (${quotaCheck.state.sentToday}/${dailyLimit}). Reset à minuit UTC.`;
    await onEvent?.({
      kind: 'quota',
      sentToday: quotaCheck.state.sentToday,
      dailyLimit,
      consecutiveFails: quotaCheck.state.consecutiveFails,
      cooldownUntil: quotaCheck.state.cooldownUntil,
    });
    for (let i = 0; i < handles.length; i++) {
      await onEvent?.({
        ...
        index: i,
        handle: handles[i]!,
        reason: quotaCheck.reason === 'cooldown' ? 'cooldown_triggered' : 'daily_limit',
      });
    }
    const final: SendXDMsOutput = {
      ...
      sentCount: 0,
      failedCount: 0,
      skippedCount: handles.length,
      stoppedEarly: true,
      cost: { userLookupCalls: 0, dmSendCalls: 0, dmSendSuccesses: 0, costUsdEstimate: 0 },
    };
    // eslint-disable-next-line no-console
    console.warn(`[send_x_dms] BLOCKED: ${reasonText}`);
  ...
  await onEvent?.({
    ...
    delayMinMs: delayMin,
    delayMaxMs: delayMax,
    quotaSentToday: quotaCheck.state.sentToday,
    dailyLimit,
  });
  ...
  for (let i = 0; i < handles.length; i++) {
    ...
    try {
      ...
      dmSendSuccesses++;
      // Persist ✓ in the quota state — daily counter + reset consecutive fails.
      const newQuota = await recordSuccess();
      await onEvent?.({
        kind: 'quota',
        sentToday: newQuota.sentToday,
        dailyLimit,
        consecutiveFails: newQuota.consecutiveFails,
        cooldownUntil: newQuota.cooldownUntil,
      });
      const result: DMResult = {
        handle,
        status: 'sent',
      ...
      results.push(result);
      // eslint-disable-next-line no-console
      console.log(`[send_x_dms] ${i + 1}/${handles.length} → @${handle} ✓ sent (event=${dm.dmEventId}) · today=${newQuota.sentToday}/${dailyLimit}`);
      await onEvent?.({ kind: 'result', index: i, result });
      // Stop hard if we've hit the daily cap during this run.
      if (newQuota.sentToday >= dailyLimit) {
        stoppedEarly = true;
        for (let j = i + 1; j < handles.length; j++) {
          await onEvent?.({ kind: 'skipped', index: j, handle: handles[j]!, reason: 'daily_limit' });
        }
        break;
      }
    } catch (e) {
      ...
      if (failureKind === 'x_refused') {
        try {
          ...
          if (verify.exists) {
            // UI-level classification only: show "✓ verified" so the user
            // knows the DM probably landed despite the 403. But DO NOT count
            // this toward the daily quota — we can't distinguish a new send
            // from an existing prior conversation, so false positives would
            // wrongly bump the counter. Daily quota counts ONLY confirmed
            // 200 OK from X (real `sendDm` success path above).
            status = 'likely_sent';
            dmSendSuccesses++; // count for the targetSuccesses gate (local)
            verifyNote = ` · verified: convo has ${verify.eventCount ?? 'some'} events (not counted in daily quota)`;
          }
        } catch {
      ...
      // If the failure is a real X refusal (not a lookup error), bump the
      // consecutive-failures counter. After N in a row, stop-loss the run.
      if (status === 'failed' && failureKind === 'x_refused') {
        const { state: q, cooldownTriggered } = await recordFailure({
        ...
        if (cooldownTriggered) {
          ...
          // eslint-disable-next-line no-console
          console.warn(
            `[send_x_dms] STOP-LOSS triggered (${q.consecutiveFails} 403s in a row). Cooldown for ${cooldownMinutes} min.`,
          );
          for (let j = i + 1; j < handles.length; j++) {
            await onEvent?.({ kind: 'skipped', index: j, handle: handles[j]!, reason: 'cooldown_triggered' });
          }
          break;
        }
      }
