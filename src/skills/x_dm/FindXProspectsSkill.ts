...
export const FindXProspectsInputSchema = z.object({
  ...
  maxFollowers: z
    ...
    .default(10_000)
    .describe(
      "Cap upstream sur le nb de followers. Appliqué dès le retour Apify (avant tout autre filtre). Default 10 000 — les petits comptes ont DMs ouverts plus souvent + meilleur taux de réponse pour cold outreach.",
    ),
});
  ...
  implements BaseSkill<FindXProspectsInput, FindXProspectsOutput>
{
  ...
  async execute(
    ...
    const maxFollowers = input.maxFollowers ?? 10_000;
    ...
    const runIteration = async (
      ...
      let res;
      try {
        res = await searchXUsersByQuery({ searchTerms: queryTerms, maxItems, maxFollowers });
      } catch (e) {
